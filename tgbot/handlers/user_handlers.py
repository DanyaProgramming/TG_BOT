    ...
    try:
        ...
        with open(photo_path, 'rb') as photo:
            await message.answer_photo(
                photo=types.BufferedInputFile(photo.read(), filename="main_menu.jpg"),
            )
        
        # 2️⃣ Отправляем текст + кнопки (можно редактировать)
        await message.answer(
            text=Templates.Navigation.MenuNavigation.Default.text(),
